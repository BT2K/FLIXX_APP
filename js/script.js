...
const globalState = {
  ...
  api: {
    apiKey: '4cd41e964ccf6d5c0606124a17235e0e',
...
function displaySearchResults(results) {
  ...
  results.forEach((result) => {
    ...
    `;
    document.getElementById('search-results-heading').innerHTML = `
 <h2>${results.length} of ${globalState.search.totalResults} results for <span class='term-color'>${globalState.search.term}</span></h2>
    `;
  ...
  results.forEach((show) => {
    ...
    div.innerHTML = `
          ...
          <div class="card-body">
            ...
            <p class="card-text">
              <small class="text-muted">First on Air: ${
                show.first_air_date
              }</small>
            </p>
          </div>
...
//Display Backdrop on details pages for Movies and Shows
function displayBackgroundImage(type, path) {
  const overlayDiv = document.createElement('div');
  overlayDiv.style.backgroundImage = `url(https://image.tmdb.org/t/p/original/${path})`;
  overlayDiv.style.backgroundSize = 'cover';
  overlayDiv.style.backgroundPosition = 'cover';
